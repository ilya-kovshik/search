...
export class SearchInputController {
  ...
  constructor(tagName: string, usersModel: IUsersModel) {
    ...
    SearchInputController.addEventListeners = (rootNode: HTMLElement) =>
      this.addEventListeners(rootNode);
    this.getSelectedItem = (id: string) =>
      SearchInputComponent.getSelectedItem(id);
  }
  ...
  private addShowListButtonEvents(
    ...
    const showListButton: HTMLInputElement | null = shadowRoot.querySelector(
      `.${templateConfig.ctrlButtons}__show`
    );
    ...
    showListButton?.addEventListener("click", async () => {
      ...
      const event: CustomEvent = new CustomEvent("showListButtonClick", {
        detail: {
          data,
          isItemSelectedCalbck: this.usersModel.isItemSelected.bind(
            this.usersModel
          )
        }
      });
  ...
  private addSearchInputEvents(
    ...
    const searchInput: HTMLInputElement | null = shadowRoot.querySelector(
      `#${templateConfig.datalistId}__input`
    );
  ...
  private addClearSelectionButtonEvents(
    ...
    const clearSelectionButton: HTMLInputElement | null = shadowRoot.querySelector(
      `.${templateConfig.ctrlButtons}__clear-selection`
    );
