...
async function createSearchComponent(): Promise<string> {
  ...
  const searchDatalistController: SearchDatalistController = new SearchDatalistController(
    "search-datalist",
    model
  );
  const searchInputController: SearchInputController = new SearchInputController(
    "search-input",
    model
  );
